...
#define CM_SLPOUT                                                              \
  0x11 // Exit sleep & booster on, also performs some self-diagnostic
...
#define CM_PWCTR1                                                              \
  0xC0 // Power control setting 1 - VRH, set GVDD (Gamma reference voltage)
#define CM_PWCTR2                                                              \
  0xC1 // Power control setting 2 - VGH and VGL supply power level
#define CM_PWCTR3                                                              \
  0xC2 // Power control setting 3 - Op-amp current in normal mode, full colours
#define CM_PWCTR4                                                              \
  0xC3 // Power control setting 4 - Op-amp current in idle mode, 8 colours
#define CM_PWCTR5                                                              \
  0xC4 // Power control setting 5 - Op-amp current in partial mode, full colours
...
#define CM_GMCTRP1                                                             \
  0xE0 // Gamma ('+' polarity) Correction Characteristics Setting
#define CM_GMCTRN1                                                             \
  0xE1              // Gamma ('-' polarity) Correction Characteristics Setting
#define CM_GCV 0xFC // Gate Pump Clock Frequency Variable

// MADCTL arguments/bit masks
#define CM_MADCTL_MY                                                           \
  0x80 // Mirror Y-axis         } These 3 bits of MADCTL control
#define CM_MADCTL_MX 0x40 // Mirror X-axis         } the write/read direction,
#define CM_MADCTL_MV                                                           \
  0x20 // Page/column selection } used to rotate/flip the screen.
#define CM_MADCTL_ML 0x10 // Vertical order - LCD vertical refresh direction
#define CM_MADCTL_BGR                                                          \
  0x08 // RGB-BGR read order from LCD controller to LCD panel. Set according to
       // the LCD panel specs.
...
#define CS_SET_HIGH                                                            \
  HWREG(LCD_CS_BASE + (LCD_CS_PIN << 2)) = LCD_CS_PIN         // 0x40064010
#define DC_SET_LOW HWREG(LCD_DC_BASE + (LCD_DC_PIN << 2)) = 0 // 0x40062020
#define DC_SET_HIGH                                                            \
  HWREG(LCD_DC_BASE + (LCD_DC_PIN << 2)) = LCD_DC_PIN // 0x40062020

#define SSIPUT(x)                                                              \
  while (!(HWREG(LCD_SSI_BASE + SSI_O_SR) & SSI_SR_TNF)) {                     \
  }                                                                            \
...
#define COLOR24TO16(x)                                                         \
  ((((x) & 0x00F80000) >> 8) | (((x) & 0x0000FC00) >> 5) |                     \
   (((x) & 0x000000F8) >> 3))
...
void SysTickISR(void) { tickCounter++; }
...
uint32_t SysTickGetTicks(void) { return tickCounter; }
...
static inline void WriteRegister(uint_fast8_t command, uint_fast8_t parameter) {
  // Data Command low indicates incoming command, Chip Select low initiates the
  // receiver.
  DC_SET_LOW;
  CS_SET_LOW;
...
static inline void WriteRegister2(uint_fast8_t command, uint_fast8_t parameter1,
                                  uint_fast8_t parameter2) {
  // Data Command low indicates incoming command, Chip Select low initiates the
  // receiver.
  DC_SET_LOW;
  CS_SET_LOW;
...
// Sets the start and boundaries of the column and row pointers in the display
// controller. Any data written after a RAMWR command (and before any other
// command) will be stored in GRAM at the current column-row pointer, which in
// turn will increment on each write according to the display orientation, i.e
// the configured MADCTL flags. Basically, tell the display controller the
// boundary of where we want to draw pixels. x0      Top-left corner of the
// boundary. Given as param 1 & 2 to CASET, is written to SC[15:0]. y0 Top-left
// corner of the boundary. Given as param 1 & 2 to RASET, is written to
// SP[15:0]. x1      Bottom-Right corner of the boundary. Given as param 3 & 4
// to CASET, is written to EC[15:0]. y1      Bottom-Right corner of the
// boundary. Given as param 3 & 4 to RASET, is written to EP[15:0].
static void SetWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
  // There's no need to check the coordinates are within the screen area, the
  // lcd controller will simply play along and discard any data written to a
  // column or row outside of its configured limits. However the datasheet seems
  // pretty adamant that x1 >= x0 and y1 >= y0, so why not, let's add a check.
  // Also all x and y must be >= 0, hence unsigned.
  ...
  // The CFAF128128B-0145T LCD display is 128x128 pixels, however its ST7735S
  // controller only supports one square panel size at 132x132 pixels. The GRAM
  // table is, of course, not aligned to one of the corners, which means the
  // outer-most rows and columns do not actually correspond to any pixels. This
  // switch adjusts the draw window to compensate.
...
static void CF128x128x16_ST7735SPixelDraw(void *pvDisplayData, int32_t i32X,
...
// i32BPP           The number of bits per pixel, must be 1, 4 or 8 (why is this
// an i32? why TI?) pui8Data         Pointer to the pixel data. For 1 and 4 BPP
// formats, the MSB represents the left-most pixel pui8Palette      Pointer to
// the palette used to draw the pixels
static void CF128x128x16_ST7735SPixelDrawMultiple(
    void *pvDisplayData, int32_t i32X, int32_t i32Y, int32_t i32X0,
    int32_t i32Count, int32_t i32BPP, const uint8_t *pui8Data,
    const uint8_t *pui8Palette) {
  uint32_t data;

  // The third argument is fine even if (i32X + i32Count) > LCD_HORIZONTAL_MAX.
  // The ST7735S controller will happily set its row and column pointers outside
  // of the display area, where it will simply ignore any data written while the
  // column/row pointers are OOB. This should let oversized draws or long text
  // overflow outside the bounds of the display area.
  ...
  case 1:
    while (i32Count) // While we have pixels to draw
    {
      ...
      for (; (i32X0 < 8) && i32Count;
           i32X0++, i32Count--) // Loop through those bytes
      {
        WriteData16(
            ((uint32_t *)pui8Palette)[(data >> (7 - i32X0)) & 1]); // oof
      }
  ...
  case 4:
    ...
    case 0:
      while (i32Count) {
        ...
        data = (*(uint32_t *)(pui8Palette + data) &
                0x00FFFFFF); // Get the corresponding 24-bit colour entry
        WriteData16(
            COLOR24TO16(data)); // Translate to 16-bit 5-6-5 color and transmit
        i32Count--;

        if (i32Count) {
        case 1: // Yep, this is some fuckery. Google "Duff's device"
          data = (*pui8Data++ & 15) * 3; // Get the lower nibble of the next
                                         // byte, then same deal as above
  ...
  case 8:
    while (i32Count--) {
      ...
      data = *(uint32_t *)(pui8Palette + data) &
             0x00FFFFFF; // Get the 24-bit colour for said byte
      WriteData16(
          COLOR24TO16(data)); // 24bit to 5-6-5bit colour, and away it goes.
    }
    break;

  case 16: // Native format for this screen. Doesn't conform to grlib, but
           // useful for images.
    while (i32Count--) {
      WriteData16(*(
          (uint16_t *)
              pui8Data)); // Cast to pointer to uint16_t, dereference for value
...
static void CF128x128x16_ST7735SLineDrawH(void *pvDisplayData, int32_t i32X1,
...
// ui32Value        The line colour
static void CF128x128x16_ST7735SLineDrawV(void *pvDisplayData, int32_t i32X,
...
static void CF128x128x16_ST7735SRectFill(void *pvDisplayData,
  ...
  // The LCD is configured for only 16 bits per pixel however, so we only need
  // to send as much. This assumes the colour value has already been converted,
  // otherwise we'll be sending 2/3 colour bytes.
  uint_fast8_t highByte = (uint_fast8_t)(ui32Value >> 8);
  uint_fast8_t lowByte = (uint_fast8_t)(ui32Value & 0xFF);

  // Make sure data/command pin is high and set chip-select low to enable
  // sending data
  GPIOPinWrite(LCD_DC_BASE, LCD_DC_PIN, LCD_DC_PIN);
  GPIOPinWrite(LCD_CS_BASE, LCD_CS_PIN, 0);

  // The size calculation is bottom-right inclusive (hence the +1's) as per the
  // grlib documentation.
...
// In this case, the LCD is configured for 16-bit colour, so we need 5-6-5 bit
// RGB. We still want it packed into a 32-bit int so it'll adhere to the driver
// API.
static uint32_t CF128x128x16_ST7735SColorTranslate(void *pvDisplayData,
                                                   uint32_t ui32Value) {
  // Mask out the most significant bits for each colour, shift them to fit
  // inside 16-bits and OR it all together.
  return COLOR24TO16(ui32Value);
}

// Used for displays without a frame buffer, writes changes in RAM to the
// display. Not used by this driver, but is still a part of the API.
...
static void SSIInit(uint32_t sysClock) {
  ...
  // Configure the SSI pins as alternate function pins, which lets the SSI
  // peripheral control them
  // GPIOPinTypeSSI(LCD_SSI_GPIO_BASE, (LCD_SSI_CLK_PIN | LCD_SSI_FSS_PIN |
  // LCD_SSI_TX_PIN | LCD_SSI_RX_PIN) );
  GPIOPinTypeSSI(LCD_SSI_GPIO_BASE, (LCD_SSI_CLK_PIN | LCD_SSI_TX_PIN));

  // Up the drive from the default 2ma to 4ma on TX/DAT0 and SCL for sharper
  // edges. Probably not necessary, but they looked a bit sluggish at 15Mhz on
  // my scope.
  GPIOPadConfigSet(LCD_SSI_GPIO_BASE, (LCD_SSI_CLK_PIN | LCD_SSI_TX_PIN),
                   GPIO_STRENGTH_4MA, GPIO_PIN_TYPE_STD);

  // Since CS and DC are active low, and should stay high most of the time,
  // configure them with pull-ups. Again, probably not strictly necessary, but
  // eh, it sounds like good practice.
  GPIOPadConfigSet(LCD_CS_BASE, LCD_CS_PIN, GPIO_STRENGTH_2MA,
                   GPIO_PIN_TYPE_STD_WPU);
  GPIOPadConfigSet(LCD_DC_BASE, LCD_DC_PIN, GPIO_STRENGTH_2MA,
                   GPIO_PIN_TYPE_STD_WPU);

  // Set SSI to 15Mhz, master mode and 8 bit data width.
  SSIConfigSetExpClk(LCD_SSI_BASE, sysClock, SSI_FRF_MOTO_MODE_0,
                     SSI_MODE_MASTER, 15000000, 8);

  // Enable the SSI module
  SSIEnable(LCD_SSI_BASE);

  // Make sure the RX FIFO is empty. Other implementations do this so presumably
  // it's important.
  uint32_t trash;
...
void CF128x128x16_ST7735SInit(uint32_t sysClock) {
  ...
  // Timing requirements are something like 10�s minimum, shorter than 5� and it
  // rejects the reset. Also, there's a 5ms wait after reset before any commands
  // can be issued, and 120ms before SLPOUT can be sent.
  GPIOPinWrite(LCD_RST_BASE, LCD_RST_PIN, 0);
  SysTickDelayMs(1);
  GPIOPinWrite(LCD_RST_BASE, LCD_RST_PIN, LCD_RST_PIN);
  SysTickDelayMs(120);

  // Software reset. Seems unecessary, but it changes a few registers from their
  // HW reset values.
  ...
  // Set the framerate during normal operation See ST7753S datasheet for
  // details. This command code (0xB1) is switched with power control options on
  // the (supposedly) equivalent HX8353. It should be fine though, SETPOWER
  // (0xB1) on HX8353 is a restricted extended command, meaning unless the EXTC
  // pin is tied low, or a correct PASSWDEN command is given, it will register
  // as a no-op.
  ...
  // Options are 0x03 for 12-bit, 0x05 for 16-bit or 0x06 for 18-bit, but only
  // really 16-bit is practical for SPI use.
  WriteRegister(CM_COLMOD, 0x05);
  SysTickDelayMs(10);

  // Memory data access control - set BGR read order from LCD controller to LCD
  // panel. This is an internal setting for how the LCD panel wants its pixel
  // values served from the controller. The controller still wants pixels in the
  // order of R-G-B from us.
  WriteRegister(CM_MADCTL, CM_MADCTL_BGR);

  // Partial mode off, go to normal mode.
  // Partial mode segments the GRAM into 128x128 pages, so you can write a full
  // screen to GRAM before displaying it. Normal mode updates the display on
  // write, but allows fancy stuff like scrolling pages.
  ...
  // Overwrite the frame buffer in the LCD. The buffer keeps through resets, but
  // contains garbage after power loss.
...
void CF128x128x16_ST7735SClear(uint32_t colour) {
  // blank the display
  tRectangle displayArea = {0, 0, LCD_HORIZONTAL_MAX, LCD_VERTICAL_MAX};

  // If there's any data above 16 bits, it must be a 24 bit colour, in which
  // case translate it to 16 bit. Any 24 bit colour where the red channel = 0
  // would still get through, but oh well.
...
const tDisplay g_sCF128x128x16_ST7735S = {sizeof(tDisplay),
                                          0,
                                          LCD_HORIZONTAL_MAX,
                                          LCD_VERTICAL_MAX,
